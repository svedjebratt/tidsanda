<script lang="ts">
	...
	let actionName = $derived(
		state.status === 'running' ? 'Pause' : state.status === 'paused' ? 'Resume' : `Start ${phaseName}`
	);
	let elapsedPercent = $derived(
		Math.min(
			...
			Math.max(
				0,
				((pomodoroDurations[state.phase] - state.remainingMs) / pomodoroDurations[state.phase]) * 100
			)
		)
	);
...
<main>
	<div>
		...
		<section class:focus={state.phase === 'focus'} class:break={state.phase === 'break'}>
			<p class="status">{state.status === 'paused' ? 'Paused' : state.status === 'waiting' ? 'Ready' : 'Running'}</p>
			<h1>{phaseName}</h1>
...
<style lang="scss">
	section {
		...
		background: linear-gradient(180deg, var(--accent-soft), var(--col-white) 58%);
		text-align: center;
